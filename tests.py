...
class TestTask(unittest.TestCase):
    def test_find_diff_extremum(self):
        input_datas = [
            ...
            [[[{'A': 3, 'B': 2}, {'A': 1, 'B': 4}], 'A', 'B', False], [{'A': 1, 'B': 4}, -3]],
            [[[{'A': 3, 'B': 2}, {'A': 1, 'B': 4}], 'A', 'B', True], [{'A': 3, 'B': 2}, 1]],
        ...
        for (datas, *keys, max_min), expected in input_datas:
            ...
            self.assertListEqual(expected, list(calculated))
...
class TestParser(unittest.TestCase):
    ...
    def parse_and_compare(self, lines, expected):
        datas = parse_data('\n'.join(lines))
    ...
    def test_parsing_weather(self):
        ...
        self.parse_and_compare(lines, expected)

    def test_parsing_football_results(self):
        lines = [
